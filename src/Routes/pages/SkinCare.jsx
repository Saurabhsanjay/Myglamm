...
const SkinCare = () => {
  ...
  bgGradient='linear(to-l, #7928CA, #FF0080)' alignSelf='center' fontSize='2xl'  as='b'>SKIN CARE</Text>
        ...
        <SimpleGrid px={{base:2,sm:10}} columns={{ base: 2,sm:3,md:3,lg:4}}  gap={6}>
        {
                AllData.SkinCare.map((el) => {
                    return (
            <Box borderRadius={10} boxShadow='rgba(0, 0, 0, 0.24) 0px 3px 8px' h={'full'} >
                ...
                <Box px='2' >
                    ...
                    <Text pt={1} pb='1' color='gray.500'> <span style={{ color: "red" }}>★</span>  {randomforrating1}.{randomforrating2} | {random} reviews</Text>
                    <Text py={2} fontWeight='medium'>₹ {el.selection4}</Text>
                </Box>
                <Spacer/>
                <Box py={1} px='2' display='flex' w={'full'} >
